...
pol_fn_gr = a_1(pol_indx);


% CUBIC INTERPOLATION ITERATION WITH 50 GRIDS
...
% CUBIC INTERPOLATION ITERATION WITH 800 GRIDS
...
% ADJUST POLICY FUNCTION BY METHOD SUGGESTED BY YOU IN THE EMAIL
pol_indx_1=zeros(5, num_a_pr, 2);
pol_shr_1=zeros(5, num_a_pr, 2);
for ii = 1:5
     ii;
        for jj = 1:num_a_pr
            x1=find(a_pr>pol_fn(ii,jj), 1);
            x2=x1-1;
            x3=(pol_fn(ii,jj)-a_pr(x2))/(a_hi/(num_a_pr-1));
...
% CALCULATE INVARIANT DISTRIBUTION WITH ADJUST POLICY FUNCTION
Mu = zeros(m,num_a_pr);
...
while mu_tol > 1e-08
    ...
    for ii = 1:length(emp_ind)
        apr_ind_1 = pol_indx_1(emp_ind(ii), a_ind(ii), 1); 
        apr_ind_2 = pol_indx_1(emp_ind(ii), a_ind(ii), 2); 
        
        apr_shr_1 = pol_shr_1(emp_ind(ii), a_ind(ii), 1); 
        apr_shr_2 = pol_shr_1(emp_ind(ii), a_ind(ii), 2); 
        
        MuNew(:, apr_ind_1) = MuNew(:, apr_ind_1) + ...
            apr_shr_1*(PI(emp_ind(ii), :) * Mu(emp_ind(ii), a_ind(ii)) )';
        MuNew(:, apr_ind_2) = MuNew(:, apr_ind_2) + ...
            apr_shr_2*(PI(emp_ind(ii), :) * Mu(emp_ind(ii), a_ind(ii)) )';
    end

    mu_tol = max(abs(abs(MuNew(:) - Mu(:))));
